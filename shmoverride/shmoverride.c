...
static void shmat_mfns(struct shm_args_hdr *shm_args, uint8_t **map,
                       struct info *info) {
    xen_pfn_t *pfntable;
    uint32_t i;
...
static void shmat_grant_refs(struct shm_args_hdr *shm_args, uint8_t **map,
                             struct info *info) {
...
void *shmat(int shmid, const void *shmaddr, int shmflg)
{
    ...
    case SHM_ARGS_TYPE_MFNS:
        shmat_mfns(shm_args, &fakeaddr, info);
        break;
    case SHM_ARGS_TYPE_GRANT_REFS:
        shmat_grant_refs(shm_args, &fakeaddr, info);
        break;
    default:
        errno = EINVAL;
    }
